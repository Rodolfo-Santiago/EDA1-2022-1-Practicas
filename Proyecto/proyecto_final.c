// Estruturas de datos y algoritmos I
// Proyecto final: Red social en lenguaje C
// Equipo 7
// Última actualización: 8/12/21 21:55
...
void comprobar_no_repetidos(struct Datos*);
void comparar_usuarios(struct Datos*, char*);
void comparar_contrasena(struct Datos* info, char* contrasena);
...
void comprobar_no_repetidos(struct Datos* info)
{
	int i;
...
void iniciar_sesion(struct Datos* info)
{
  int i;
	...
	// comparar usuarios
	comparar_usuarios(info, usuario);

	printf("Ingrese su contraseña: ");
	scanf("%s", contrasena);

	// validar contraseña
	comparar_contrasena(info, contrasena);

	menu_cuenta(info);
}

void comparar_usuarios(struct Datos* info, char* usuario)
{
	int i;
