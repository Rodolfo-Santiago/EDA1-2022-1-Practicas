...
// Equipo 7
// Última actualización: 22/10/21 14:39


/*****************Implementación de una lista doblemente ligada circular*****************/
...
void quitar_persona( )
{
	...
	if (inicio == (*inicio).siguiente)
	{
		printf("\n\n%s era la unica persona en el juego, por lo que ha sido eliminado.", (*inicio).nombre);
...
{	
	...
	if (posicion_inicio > (numero_personas / 2) + 1)
	{
		for (i = 0; i < (numero_personas - posicion_inicio) + 1; i++)
		{
			temp_1 = (*temp_1).anterior;
		}
	}
	else
	{
	...
	for (i = 1; i < numero_personas; ++i)
	{
		...
		if (numero_saltos > 0)
		{
			if (numero_saltos % personas_en_juego > (personas_en_juego / 2) + 1)
			{
				for (k = 0; k < (personas_en_juego - ((numero_saltos - var_auxiliar) % personas_en_juego)); ++k)
				{
			...
			{
				for (k = 1; k <= (numero_saltos - var_auxiliar) % personas_en_juego; ++k)
				{
		...
		{
			if (numero_saltos % personas_en_juego < ((0 - personas_en_juego) / 2) - 1)
			{
				for (k = 0; k > (0 - personas_en_juego - ((numero_saltos + var_auxiliar) % personas_en_juego)); --k)
				{
					temp_1 = (*temp_1).siguiente_vivo;
				}
			}
